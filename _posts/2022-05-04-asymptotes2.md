...
{% equation %}x^{2}+4\le x^{2}+2x+4=\left(x+2\right)^{2}{% endequation %}
...
באופן דומה, כשיש לי את אי השוויון {% equation %}\sqrt{x^{2}+4}\le\sqrt{\left(x+2\right)^{2}}{% endequation %} ואני רוצה לבצע את המעבר {% equation %}\sqrt{\left(x+2\right)^{2}}=x+2{% endequation %} אני מתבסס על ההנחה ש-{% equation %}x+2\ge0{% endequation %}. אם {% equation %}x<-2{% endequation %} (כפי שקורה כאשר {% equation %}x\to-\infty{% endequation %}) נקבל דווקא את ההפך, כלומר {% equation %}\sqrt{\left(x+2\right)^{2}}=-\left(x+2\right){% endequation %}, ומכאן שנקבל:
