...
{% equation %}p\left(5k+4\right)\equiv0\left(\text{mod 5}\right){% endequation %}

{% equation %}p\left(7k+5\right)\equiv0\left(\text{mod 7}\right){% endequation %}

{% equation %}p\left(11k+6\right)\equiv0\left(\text{mod 11}\right){% endequation %}
...
ה-{% equation %}\equiv0\left(\text{mod 5}\right){% endequation %} מייצג, במקרה זה, את הטענה "מתחלק ב-5". למה לכתוב את זה כל כך מסובך? כי לסימון {% equation %}a\equiv b\left(\text{mod n}\right){% endequation %} יש משמעות כללית יותר שאותה רוב המתמטיקאים מכירים, ונוח להשתמש בסימון הזה גם למטרה זו, וגם נראה הרבה משוואות כאלו בהמשך הפוסט. פורמלית, המשמעות של {% equation %}a\equiv b\left(\text{mod n}\right){% endequation %} היא "{% equation %}a-b{% endequation %} מתחלק ב-{% equation %}n{% endequation %}", או בניסוח שקול - "כאשר מחלקים את {% equation %}a{% endequation %} ב-{% equation %}n{% endequation %} מתקבלת אותה שארית כמו זו שמתקבלת כאשר מחלקים את {% equation %}b{% endequation %} ב-{% equation %}n{% endequation %}".
...
רמנוג'אן גם הוסיף את ההערה "נראה כי אין תכונות פשוטות דומות עבור אף מודולוס ראשוני פרט לאלו" (מודולוס הוא מה שמופיע אחרי ה-{% equation %}\text{mod}{% endequation %} - מה שבודקים את החלוקה בו). ההערה הזו נותרה תעלומה זמן רב - לא ברור כיצד ניתן בדיוק לנסח אותה באופן מתמטי, ואפשר רק לסמוך על האינטואיציה של רמנוג'אן - והכתבה ב-Ynet עוסקת במאמר שבין היתר מציע לה הסבר מסויים.

שנים אחרי רמנוג'אן הצליח המתמטיקאי אטקין למצוא משוואות מודולריות נוספות באותה רוח. למשל:

{% equation %}p\left(1977147619k+815655\right)\equiv0\left(\text{mod 19}\right){% endequation %}
...
{% equation %}p\left(13^{3}n+1007\right)\equiv6\cdot p\left(13n+6\right)\left(\text{mod 13}\right){% endequation %}
...
הטענה הכללית יותר מנוסחת באופן קצת יותר מסורבל ועמוס הנחות, אבל ככה זה - כאמור, אל תצפו שהכל יהיה נוסחאות נוצצות ופשוטות. אצטט אותה מהמאמר: עבור ראשוניים {% equation %}5\le l\le31{% endequation %} (למה {% equation %}l{% endequation %} ולא {% equation %}p{% endequation %}, שבדרך כלל מסמן ראשוני? ובכן, כי {% equation %}p{% endequation %} מסמן את פונקציית החלוקה ואנחנו לא רוצים להתבלבל) וחזקה {% equation %}m\ge1{% endequation %} מתקיים שאם {% equation %}b_{1}\equiv b_{2}\left(\text{mod }2\right){% endequation %} עבור טבעיים {% equation %}b_{2}&gt;b_{1}\ge2m-1{% endequation %} אז עבור מספר {% equation %}A{% endequation %} מסויים שתלוי ב-{% equation %}l,b_{1},b_{2},m{% endequation %} מתקיימת המשוואה

{% equation %}p\left(\frac{l^{b_{2}}k+1}{24}\right)\equiv A\cdot p\left(\frac{l^{b_{1}}k+1}{24}\right)\left(\text{mod }l^{m}\right){% endequation %}, לכל {% equation %}k{% endequation %} טבעי שעבורו השבר יוצא מספר שלם (אין משמעות לפונקצית החלוקה על שברים...)
